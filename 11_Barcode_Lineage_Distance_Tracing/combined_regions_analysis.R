...
#         CT wide table. Samples are matched between C and T by stripping the
#         _S## suffix (since C and T are separate sequencing runs with
#         different S-indices). Barcodes are prefixed "C:" or "T:" so identical
#         sequences in the two assays don't collide.
...
# ---- USER SETTINGS ------------------------------------------------------
c_file <- "C:/Users/fc809/Downloads/C_regions_results/combined_barcodes_C_regions.tsv"   # <- edit to your C-region wide file
t_file <- "C:/Users/fc809/Downloads/T_region_result/combined_barcodes_T_region.tsv"   # <- edit to your T-region wide file

combined_file <- "combined_barcodes_wide_CT.tsv"
...
# =========================================================================
read_and_prep <- function(path, region_prefix) {
  ...
  new_names <- stringr::str_remove(sample_cols, "_S\\d+$")
  dups <- new_names[duplicated(new_names)]
  if (length(dups) > 0) {
    warning("After stripping _S##, duplicate sample names in ", region_prefix,
...
c_wide <- read_and_prep(c_file, "C")
message("Reading T-region: ", t_file)
t_wide <- read_and_prep(t_file, "T")
...
# Helper: build the per-sample combined frequency matrix for a set of samples,
# with C and T normalized SEPARATELY per sample then averaged 50/50.
build_freq_matrix <- function(m_sub, sample_cols_local) {
  # m_sub is a tibble with a 'barcode' column and the given sample columns.
  # Returns a matrix of frequencies (rows = barcodes, cols = samples).
  ...
  # Average (halve and stack); if one region has 0 UMIs in a sample, that
  # half is all zeros - renormalize so the present region gets full weight.
  ...
  rownames(Fmat) <- m_sub$barcode   # preserves C: / T: prefix for sanity checks
  Fmat
}
...
run_destination <- function(dest_pattern, dest_short) {
  ...
  for (m in sort(unique(dest_meta$mouse))) {
    ...
    # Restrict to barcodes detected in any of this mouse's relevant samples
