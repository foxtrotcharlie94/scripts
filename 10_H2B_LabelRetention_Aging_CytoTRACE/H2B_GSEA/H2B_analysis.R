...
##
## Gene sets built from top/bottom 200 of that ranking:
##   LR_up   = genes higher in label-RETAINING HSCs
##   LR_down = genes higher in label-LOSING HSCs
##
...
OUTDIR  <- "C:/Users/fc809/Downloads/H2B_GSEA"
N_GENES <- 1000
...
cat("LR_up   (label-retaining enriched):", length(lr_up),   "genes\n")
cat("LR_down (label-losing enriched):   ", length(lr_down), "genes\n\n")
...
plot_h2b_gsea <- function(gsea_obj, title, outpath) {
  ...
  p[[1]] <- p[[1]] +
    scale_color_manual(
      ...
      labels = c(
        "LR_up"   = paste0("LR-up   NES=", nes_up, "  FDR=", fdr_up),
        "LR_down" = paste0("LR-down NES=", nes_dn, "  FDR=", fdr_dn)
      )
    ) +
...
p_dot <- ggplot(gsea_summary,
  ...
  scale_fill_gradient2(low = "#1A237E", mid = "white", high = "#B71C1C",
                       midpoint = 0, limits = c(-2.5, 2.5), name = "NES") +
