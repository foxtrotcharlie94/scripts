## ============================================================
##  GSEA sensitivity analysis — LT-HSCs
...
FIXED_THRESH <- 13700     # fixed nUMI threshold (cases 3 & 5)
...
filter_min_cells <- function(meta_sub, group_var) {
  ...
  message("    Samples surviving filter: ", paste(keep_samples, collapse = ", "))
...
for (cmp in comparisons) {
  ...
  for (case_id in names(cases)) {
    ...
    # 3. Min cells filter
    meta <- filter_min_cells(meta, cmp$group_var)
  ...
  write.csv(all_res,
            paste0("GSEA_LTHSCs_", cmp$id, "_all_results.csv"),
            row.names = FALSE)
...
ggsave("GSEA_LTHSCs_sensitivity_heatmap.pdf",
       combined, width=52, height=28, limitsize=FALSE)
ggsave("GSEA_LTHSCs_sensitivity_heatmap.png",
       combined, width=52, height=28, dpi=100, limitsize=FALSE)

message("\nDone. Saved: GSEA_LTHSCs_sensitivity_heatmap.pdf / .png")
