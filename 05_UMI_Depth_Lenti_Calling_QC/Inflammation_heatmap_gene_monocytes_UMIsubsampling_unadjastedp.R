...
FIXED_THRESH2 <- 9227
MIN_CELLS     <- 100
FDR_CUT       <- 0.05   # unadjusted p-value threshold (not FDR)
...
## ── Helper: subsample cells per sample×group to a fixed global UMI target ─────
# target_umi must be pre-computed globally across ALL comparisons and cases
subsample_equal_umis <- function(meta_sub, group_var, target_umi) {
  result <- list()
  groups <- unique(meta_sub[[group_var]])
  for (samp in unique(meta_sub$sample)) {
    for (grp in groups) {
      cells <- meta_sub[meta_sub$sample == samp & meta_sub[[group_var]] == grp, ]
      if (nrow(cells) == 0) next
      cells <- cells[sample(nrow(cells)), ]  # shuffle
      cum_umi <- cumsum(cells$nUMI)
      keep_n  <- which(cum_umi >= target_umi)[1]
      if (is.na(keep_n)) keep_n <- nrow(cells)
      result[[paste(samp, grp)]] <- cells[seq_len(keep_n), ]
    }
  }
  do.call(rbind, result)
}

## ── Pre-compute global UMI target across ALL comparisons × cases ───────────────
# Run through every comparison×case, compute total UMI per sample×group,
# collect the minimum across all of them → this becomes the global target
message("Computing global UMI target...")
global_totals <- c()
...
for (cmp in comparisons) {
  for (case_id in names(cases)) {
    ...
    totals <- meta %>%
      ...
      summarise(total_umi = sum(nUMI), .groups = "drop") %>%
      pull(total_umi)
    
    global_totals <- c(global_totals, totals)
  }
}

GLOBAL_TARGET_UMI <- min(global_totals)
message("  Global UMI target: ", format(round(GLOBAL_TARGET_UMI), big.mark=","),
        " (min across ", length(global_totals), " sample×group×case combinations)")
...
for (cmp in comparisons) {
  ...
  for (case_id in names(cases)) {
    ...
    # Subsample to global UMI target before pseudobulking
    meta <- subsample_equal_umis(meta, cmp$group_var, GLOBAL_TARGET_UMI)
    message("  After UMI equalisation (target=",
            format(round(GLOBAL_TARGET_UMI), big.mark=","),
            "): ", nrow(meta), " cells")
...
# Save raw results
write.csv(all_deg, "inflammation_genes_DEG_equalUMI_pval_all_results.csv", row.names=FALSE)
message("\nDEG results saved.")
...
fdr_wide <- all_deg %>%
  dplyr::select(gene, col_id, PValue) %>%
  pivot_wider(names_from=col_id, values_from=PValue) %>%
  column_to_rownames("gene")
...
for (r in rownames(lfc_masked))
  for (c in colnames(lfc_masked))
    if (is.na(fdr_wide[r,c]) || fdr_wide[r,c] >= FDR_CUT)  # unadjusted p
      lfc_masked[r,c] <- NA
...
## ── Save ──────────────────────────────────────────────────────────────────────
out_stem <- "Inflammation_genes_heatmap_Monocytes_equalUMI_pval"
...
draw(ht,
     column_title    = paste0("Classical Monocytes — Inflammation genes, UMI-equalised pseudobulk (logFC, p < ", FDR_CUT, " unadjusted)"),
