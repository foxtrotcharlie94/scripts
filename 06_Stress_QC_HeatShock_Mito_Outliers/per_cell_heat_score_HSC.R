# ══════════════════════════════════════════════════════════════════════════════
#  Heat Shock Module Score Analysis — LT-HSCs
...
out_dir   <- "LT-HSCs (1)/heatshock_analysis"
...
p_ridge <- ggplot(df, aes(x = hs_score, y = sample, fill = sample, color = sample)) +
  ...
  labs(
    title    = "LT-HSC Heat Shock Module Score — Distribution per Sample",
...
p_violin <- ggplot(df, aes(x = sample, y = hs_score, fill = sample, color = sample)) +
  ...
  scale_color_manual(values = sample_colors, guide = "none") +
  labs(
    title    = "LT-HSC Heat Shock Module Score per Cell",
...
p_cdf_all <- ggplot(cdf_df, aes(x = hs_score, y = cdf, color = sample,
  ...
  labs(
    title = "LT-HSC Heat Shock Module Score — Cumulative Distribution",
...
p_rescue <- ggplot(rescue_df, aes(x = threshold, y = pct_rescued)) +
  ...
  labs(
    title    = "LT-HSC HB4: Fraction of Cells Rescued by Score Threshold",
...
p_hb4_density <- ggplot(hb4, aes(x = hs_score)) +
  ...
  labs(
    title    = "LT-HSC HB4: Heat Shock Score Distribution",
...
p_hb4 <- p_hb4_density + p_rescue +
  plot_annotation(
    title = "LT-HSC HB4: Cell-Level Rescue Analysis",
    theme = theme(plot.title = element_text(face = "bold", size = 15, color = "black"))
  )
...
p_rescue_all <- ggplot(rescue_all,
  ...
  scale_y_continuous(limits = c(0, 100), labels = function(x) paste0(x, "%")) +
  labs(
    title    = "LT-HSC Heat Shock — Fraction of Cells Below Score Threshold",
