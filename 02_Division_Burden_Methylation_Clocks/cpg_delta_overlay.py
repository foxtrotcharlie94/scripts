...
df = pd.read_csv('/tmp/gse44117/cpg_deltas.csv')

fig, ax = plt.subplots(figsize=(8, 7))
colors = {"Blood": "#4C72B0", "WLMT": "#DD8452"}
text_y = {"Blood": 0.97, "WLMT": 0.80}

for clockname in ["Blood", "WLMT"]:
...
ax.set_title("Per-CpG change: natural aging vs. forced-proliferation transplant\n(GSE44117, values are means across replicates per group)")
ax.legend(fontsize=8, loc="lower right")
plt.tight_layout()
plt.savefig('/tmp/gse44117/delta_age_vs_delta_reconst_overlay.png', dpi=150, bbox_inches='tight')
