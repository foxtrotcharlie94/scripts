...
df = pd.read_csv('/tmp/gse44117/cpg_deltas_all3.csv')

fig, ax = plt.subplots(figsize=(8.5, 7.5))
colors = {"Blood": "#B3B3B3", "WLMT": "#1B3A5C", "YOMT": "#D4A017"}
...
for clockname in ["Blood", "WLMT", "YOMT"]:
    ...
    ax.scatter(x, y, s=16, alpha=0.6, color=colors[clockname], label=f"{clockname} clock CpGs (n={len(sub)})")
    xs = np.linspace(x.min(), x.max(), 100)
    ax.plot(xs, slope * xs + intercept, color=colors[clockname], lw=2.2)
...
plt.tight_layout()
plt.savefig('/tmp/gse44117/delta_age_vs_delta_reconst_overlay4.png', dpi=150, bbox_inches='tight')
print("saved recolored overlay plot")
