# ══════════════════════════════════════════════════════════════════════════════
#  limma-voom DGE + Pathway Analysis — Five comparisons
# ALL SAMPLES INCLUDED (HB3 and HB4 not excluded)
...
#  BiocManager::install(c("limma", "edgeR", "clusterProfiler", "org.Mm.eg.db"))
#
# ══════════════════════════════════════════════════════════════════════════════
...
base_dir     <- "C:/Users/fc809/Downloads/LT-HSCs (1)"
results_base <- "C:/Users/fc809/Downloads/LT-HSCs (1)/allSamples_analysis"
...
# ── Helper: run limma-voom ────────────────────────────────────────────────────
run_voom <- function(counts, group, label1, label2) {
...
for (comp in comparisons) {
  ...
  keep        <- !is.na(group)  # all samples included
  ...
  counts     <- raw[!rownames(raw) %in% c("Lenti", "lenti"), sample_cols, drop = FALSE]
  safe_label <- gsub("[^A-Za-z0-9_]", "_", comp$title)
  
  res <- run_voom(counts, group, comp$label1, comp$label2)
